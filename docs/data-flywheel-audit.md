...
> 本次复核确认消费者边界与原审计一致；`v1.14.0` 新增基于当前历史的提问风格与学习节奏展示，但没有扩大 prompt 消费者范围，也没有改变事件持久化语义。
...
- 事实/评分路径隔离
- Best-effort 飞轮刷新
- 画像展示时从当前计划快照重算行为字段，旧 AI 诊断文本和无依据的时段偏好不会覆盖真实证据
...
- `user-profile.test.js`：覆盖提问风格阈值、样本证据与无依据文本清理
- `v1.14.0` 全量基线：Server 540/540，Client 112/112
