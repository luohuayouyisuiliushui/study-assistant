...
> 本文的文件行号和“修复前”代码块保留自 2026-07-10 原始审计，用于说明问题来源，不代表当前源码位置。2026-07-27 已逐项复核：11 项均已修复、由测试覆盖或按正确持久化语义关闭，当前无待修复项。`v1.14.0` 未修改持久层协议或依赖声明。
...
当前自动化基线为 Server `540/540`、Client `112/112`，数据完整性检查通过。与真实收费 AI 供应商相关的立即停止计费行为仍取决于上游对 HTTP abort 的实现；本项目已验证 SDK signal 透传及本地 chunk 分发停止。
