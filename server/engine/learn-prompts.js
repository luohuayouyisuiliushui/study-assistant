...
export const STABLE_DETAIL_SYSTEM_PROMPT =
  ...
  '7. **与相关知识点的联系（##）**：**必须存在的段落，不可省略**。列出当前知识点与计划内其他知识点的关联——哪些是它的前置基础、哪些是它的延伸应用。每条关系一行，格式：`- **知识点标题**：关联说明`\n\n' +
  '## 质量标准\n' +
  ...
  '- **注重实操细节**：讲解不能只停留在理论层面，必须包含具体的工具使用、命令示例、配置步骤、调试方法等实操内容。例如讲交叉编译要给出工具链安装命令和编译参数，讲 epoll 要给出核心 API 的代码框架\n' +
  ...
  '- **涉及流程、架构、状态转换、时序关系的内容，必须使用 Mermaid 图表绘制**（```mermaid ... ```），系统会自动渲染。例如流程图(graph)、时序图(sequenceDiagram)、类图(classDiagram)、状态图(stateDiagram-v2)。不满足"适合用图表"条件的才用文字描述\n' +
...
export const IMPORT_PLAN_PROMPT =
  ...
  '## 两层结构定义\n' +
  '- **Phase（学习阶段）**：最高层分组，按学习进度划分。通常 1 个，最多 3 个\n' +
